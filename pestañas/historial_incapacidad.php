...
<div class="principal">
    <section>
        ...
        <input type="hidden" name="pestaña" value="registro_incapacidad">
        <button type="submit" style="padding: 10px 20px;
          ...
          border-radius: 4px;
          cursor: pointer;
          font-size: 14px;">+ Registrar incapacidad</button>
      </form>
        <h1 class="text-2xl font-bold text-blue-600">Historial de Incapacidades</h1>
        <br>
        ...
        <div class="buscador">
            <form class="reporte_formulario" method="GET" action="">
                ...
                <input type="hidden" name="pestaña" value="historial_incapacidad">
        ...
        $sql_base = "
            FROM incapacidades
            LEFT JOIN trabajadores ON incapacidades.id_trabajador = trabajadores.id
            WHERE 1=1
        ";
        $params = [];
        $types = '';
        ...
        if (!empty($fecha_inicio) && !empty($fecha_fin)) {
            $sql_base .= " AND incapacidades.fecha_inicio >= ? AND incapacidades.fecha_fin <= ?";
            $params[] = $fecha_inicio;
            $params[] = $fecha_fin;
            $types .= 'ss';
        }
        ...
        // Datos paginados
        $sql_final = "SELECT incapacidades.fecha_inicio, incapacidades.fecha_fin, incapacidades.dias, incapacidades.tipo_incapacidad, incapacidades.observaciones, trabajadores.nombre, trabajadores.apellidos " . $sql_base . " ORDER BY incapacidades.fecha_inicio DESC LIMIT ?, ?";
        $params[] = $offset;
        ...
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($f_inicio, $f_fin, $dias, $tipo, $obs, $nombre, $apellidos);
        ...
        if ($stmt->num_rows > 0) {
            ...
            while ($stmt->fetch()) {
                echo "<tr>
                        ...
                        <td>" . htmlspecialchars($f_inicio) . "</td>
                        <td>" . htmlspecialchars($f_fin) . "</td>
            ...
            echo "No se encontraron registros de incapacidades.";
        }
