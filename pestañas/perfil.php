...
if (!empty($_FILES['key']['tmp_name'])) {
    ...
    if ($datos_existentes) {
        ...
        $stmt = $conexion->prepare("UPDATE perfil_fiscal SET logo=?, rfc=?, razon_social=?, regimen_fiscal=?, ruta_cer=?, ruta_key=?, password_key=?, codigo_postal=?, registro_patronal=?, curp=?, correo=?, telefono=?, direccion=? WHERE id=?");
        ...
        // Insertar nuevo
        $stmt = $conexion->prepare("INSERT INTO perfil_fiscal (logo, rfc, razon_social, regimen_fiscal, ruta_cer, ruta_key, password_key,codigo_postal, registro_patronal, curp, correo, telefono, direccion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
<div class="principal">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <select name="regimen_fiscal" required>
            ...
            if ($regimen_actual) {
                $res = $conexion->query("SELECT Descripcion FROM RegimenFiscal WHERE Clave = '$regimen_actual' LIMIT 1");
                if ($res->num_rows > 0) {
            ...
            $result=mysqli_query($conexion,$regimenes);
