...
<body id="analisis_compras">
    ...
    // Generar la consulta para obtener la suma de total_pesos por mes en el rango de fechas seleccionado
    $sql = "SELECT YEAR(fecha_solicitud) as year, MONTH(fecha_solicitud) as mes, SUM(total_pesos) as total_pesos
            FROM orden_compra
    ...
    while ($current <= $end) {
        ...
        $totals[$year . '-' . $month] = 0; // Inicializar con 0 en caso de que no haya compras en ese mes
        $current = strtotime("+1 month", $current);
    }

    // Rellenar los totales con los datos obtenidos de la consulta
    while ($row = $result->fetch_assoc()) {
        $totals[$row['year'] . '-' . str_pad($row['mes'], 2, '0', STR_PAD_LEFT)] = $row['total_pesos'];
    }
    ...
    <div class="contenedor__servicios">
        <h2 class="titulo">Análisis de Compras</h2>
        <form method="GET">
            ...
            <input type="hidden" name="pestaña" value="analisis_compras">
    ...
    <script>
        ...
        var graficaCompras = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: <?php echo json_encode($months); ?>,
                datasets: [{
                    label: 'Total de Compras por Mes (en Pesos)',
