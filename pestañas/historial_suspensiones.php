...
<head>
    <meta charset="UTF-8">
    <title>Historial de Suspensiones</title>
    <style>
...
<div class="principal">
    <section>
        ...
        <input type="hidden" name="pestaña" value="registro_suspenciones">
        <button type="submit" style="padding: 10px 20px;
          ...
          border-radius: 4px;
          cursor: pointer;
          font-size: 14px;">+ Registrar suspencion</button>
      </form>
        <h1 class="text-2xl font-bold text-blue-600">Historial de Suspensiones</h1>
        <br>
        ...
        <div class="buscador">
            <form class="reporte_formulario" method="GET" action="">
                ...
                <input type="hidden" name="pestaña" value="historial_suspensiones">
        ...
        $sql_base = "
            FROM suspensiones
            LEFT JOIN trabajadores ON suspensiones.id_trabajador = trabajadores.id
            WHERE 1=1
        ";
        $params = [];
        $types = '';
        ...
        if (!empty($fecha_inicio) && !empty($fecha_fin)) {
            $sql_base .= " AND suspensiones.fecha_final BETWEEN ? AND ?";
            $params[] = $fecha_inicio;
            $params[] = $fecha_fin;
            $types .= 'ss';
        }
        ...
        // Datos paginados
        $sql_final = "SELECT suspensiones.fecha_inicial, suspensiones.motivo, suspensiones.observaciones, trabajadores.nombre, trabajadores.apellidos , suspensiones.fecha_final" . $sql_base . " ORDER BY suspensiones.fecha_inicial DESC LIMIT ?, ?";
        $params[] = $offset;
        ...
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($fecha_inicio, $motivo, $obs, $nombre, $apellidos, $fecha_fin);
        ...
        if ($stmt->num_rows > 0) {
            ...
            echo "No se encontraron suspensiones registradas.";
        }
