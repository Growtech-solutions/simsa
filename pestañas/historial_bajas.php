...
<div class="principal">
    <section>
        ...
        <input type="hidden" name="pestaña" value="baja_trabajador">
        <button type="submit" style="padding: 10px 20px;
          ...
          border-radius: 4px;
          cursor: pointer;
          font-size: 14px;">+ Registrar baja</button>
      </form>
        <h1 class="text-2xl font-bold text-blue-700" style="text-align:center;">Historial de Bajas</h1>
        <br>
        ...
        <div class="buscador">
            <form class="reporte_formulario" method="GET" action="">
                ...
                <input type="hidden" name="pestaña" value="historial_bajas">
        ...
        $sql_base = "
            FROM bajas
            LEFT JOIN trabajadores ON bajas.id_trabajador = trabajadores.id
            WHERE 1=1
        ";
        $params = [];
        $types = '';
        ...
        if (!empty($fecha_inicio) && !empty($fecha_fin)) {
            $sql_base .= " AND bajas.fecha BETWEEN ? AND ?";
            $params[] = $fecha_inicio;
            $params[] = $fecha_fin;
            $types .= 'ss';
        }
        ...
        // Datos paginados
        $sql_final = "
            SELECT trabajadores.nombre, trabajadores.apellidos, bajas.fecha, bajas.causa, bajas.comentario
            " . $sql_base . " ORDER BY bajas.fecha DESC LIMIT ?, ?";
        $params[] = $offset;
        ...
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($nombre, $apellidos, $fecha, $causa, $comentario);
        ...
        if ($stmt->num_rows > 0) {
            ...
            while ($stmt->fetch()) {
                echo "<tr>
                        ...
                        <td>" . htmlspecialchars($causa) . "</td>
                        <td>" . nl2br(htmlspecialchars($comentario)) . "</td>
                    </tr>";
            }
            echo "</table>";
            ...
            echo "No se encontraron registros de bajas.";
        }
