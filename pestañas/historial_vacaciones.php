...
<div class="principal">
    <section>
        ...
        <input type="hidden" name="pestaña" value="registro_vacaciones">
        <button type="submit" style="padding: 10px 20px;
          ...
          border-radius: 4px;
          cursor: pointer;
          font-size: 14px;">+ Registrar vacaciones</button>
      </form>
        <h1 class="text-2xl font-bold text-blue-600">Historial de Vacaciones</h1>
        <br>
        ...
        <div class="buscador">
            <form class="reporte_formulario" method="GET" action="">
                ...
                <input type="hidden" name="pestaña" value="historial_vacaciones">
        ...
        $sql_base = "
            FROM vacaciones
            LEFT JOIN trabajadores ON vacaciones.id_trabajador = trabajadores.id
            WHERE 1=1
        ";
        $params = [];
        $types = '';
        ...
        if (!empty($fecha_inicio) && !empty($fecha_fin)) {
            $sql_base .= " AND vacaciones.fecha_inicio >= ? AND vacaciones.fecha_fin <= ?";
            $params[] = $fecha_inicio;
            $params[] = $fecha_fin;
            $types .= 'ss';
        }
        ...
        // Datos paginados
        $sql_final = "SELECT vacaciones.fecha_inicio, vacaciones.fecha_fin, vacaciones.dias_solicitados, vacaciones.motivo, vacaciones.estado, trabajadores.nombre, trabajadores.apellidos " . $sql_base . " ORDER BY vacaciones.fecha_inicio DESC LIMIT ?, ?";
        $params[] = $offset;
        ...
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($f_inicio, $f_fin, $dias, $motivo, $estado, $nombre, $apellidos);
        ...
        if ($stmt->num_rows > 0) {
            ...
            echo "No se encontraron solicitudes de vacaciones.";
        }
